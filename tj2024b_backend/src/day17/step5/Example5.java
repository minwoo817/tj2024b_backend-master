...
interface InterFaceTest2{
	...
	// + : 인터페이스는 클래스와 다르게 생성자가 없다. 즉] 인터페이스는 new 인터페이스(); X , 인터페이스는 인스턴스를 만들 수 없다.
		// - 생성자란? 클래스의 구성 멤버이자, 클래스로 인스턴스를 만들때 인스턴스의 초기화하는 메소드/역할
	// + static 이란? 프로그램이 시작될때 메모리 할당되고 프로그램이 끝날때 메모리 초기화[상수/전역(공유)]
		// - static 있는 멤버변수 또는 메소드는 인스턴스가 없어도 사용이 가능하다.
...
public class Example5 {
	public static void main(String[] args) {
		...
		// InterFaceTest2.method2(); , 	X
		// [4] 인터페이스의 static 정적메소드 호출 , 	O
		InterFaceTest2.method3();
		
		// [*] 인터페이스 타입은 new 인터페이스명() 할 수 없다. 즉] 인스턴스 생성 불가
