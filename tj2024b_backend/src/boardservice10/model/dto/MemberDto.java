...
*/
public class MemberDto { // DTO : 서로 다른 레이어들(MVC패턴)간의 데이터를 이동하는 객체
	// * 주로 데이터베이스 테이블의 속성과 동일하다. + 커스텀 가능
	// * 주로 DTO는 멤버변수 private 접근제한자 사용 private : 다른 클래스에서 접근 불가능
		// - getter/setter 메소드 : 접근제한된 멤버변수를 다른 클래스에서 간접 접근 하기 위한 메소드
		//- toString() 메소드 : 객체 호출시 참조(주소)값 대신 멤버변수 값을 반환하는 메소드
	// * 주로 DTO는 생성자를 기본적으로 2개(디폴트, 풀생성자), 그 외 커스텀, 생성자 : new(인스턴스/객체)를 생성할때 사용되는 초기화함수
	...
	// 2. 생성자 // 자동완성 : 클래스 { } 안에서 빈칸에 오른쪽 클릭 -> [source] -> [Generate Constructor]
		public MemberDto() {}
