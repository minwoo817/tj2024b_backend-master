...
/*
    ...
    2. SQL 코드 실행 방법
		방법1 : 단위실행
			1. 실행할 코드에 커서를 두고 CTRL+ENTER
			2. 실행할 코드에 커서를 두고 (번개I아이콘) 클릭
		방법2 : 현재 파일내 모든 코드 실행(위에서 아래 순으로)
			1. CTRL+SHIFT+ENTER
            2. (번개아이콘) 클릭
	3. SQL 코드 실행 결과
...
# create(생성) database(데이터베이스) DB명 , create database DB명
create database mydb1226;

# [4] db server의 데이터베이스 삭제하기
# drop(버림) database(데이터베이스) DB명 , drop database DB명
